...
class GitlabAPI:
    ...
    def _make_get(self,recurso,dados=""):
        servidor = "http://192.168.201.123/api/v3/%s%s?private_token=%s"%(recurso,dados,self.token)
        try:
    ...
    def _make_post(self,recurso,dados):
        servidor = "http://192.168.201.123/api/v3/%s?private_token=%s"%(recurso,self.token)
        try:
    ...
    def add_project_hook(self,url,project):
        ...
        if response.status_code == 201:
            print "Webbhook adicionada com sucesso"
        else:
...
if __name__ == '__main__':
    ...
    novo = {"name":"Leandro",
            "username":"leandro",
            "email":"leandro@dexter.com.br",
            "password":"4linux123"}
    #gitlab.create_user(novo)
    #gitlab.create_project("Kamehameha")
    #gitlab.add_project_member("leandro","Kamehameha")
